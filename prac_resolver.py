...
r = redis.Redis(host='localhost', port=6379, db=0, password="20092010")
...
def dns_response(data, c_ip):
    ...
    st = prefix + t
    import random
    st = st + str(random.randint(1, 2000))
    r.set(st, c_ip)
    ...
    index += 1
    chosen_record = record_list[index % (len(ip_list))]
...
class BaseRequestHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        try:
            ...
            self.send_data(dns_response(data, c_ip))
        except Exception:
